...
class EndMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for i in range(2, len(_map[0])):
                for j in range(2, len(_map)):
                    coord = (i, j)
                    self.objects.append(Objects.Ally(
                        prop['sprite'], prop['action'], coord, prop['sound']))
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['enemies']:
                ...
                for i in range(random.randint(0, 5)):
                    ...
                    self.objects.append(Objects.Enemy(obj_name,
                        prop['sprite'], prop, prop['experience'], coord,
                        prop["sound"]))

            for obj in self.config.keys():
                count = self.config[obj]
                for obj_type in object_list_prob:
