...
class GameSurface(ScreenHandle):
    ...
    def draw_hero(self):
        self.game_engine.hero.draw(self, self.game_engine.
                                    map, self.game_engine.sprite_size)   

    def draw_map(self):
        
        min_x, min_y = self.calculate_mins()
    ...
    def calculate_mins(self):
        x = self.game_engine.hero.position[0]
        if x - 6 <= 0:
            min_x = 0
        elif self.game_engine.hero.position[0] > len(self.game_engine.map[0]) - 5:
            min_x = len(self.game_engine.map[0]) - 11
        else:
            min_x = x - 6

        y = self.game_engine.hero.position[1]

        if y - 4 <= 0:
            min_y = 0
        elif self.game_engine.hero.position[1] > len(self.game_engine.map) - 4:
            min_y = len(self.game_engine.map[0]) - 8
        else:
            min_y = y - 4
    ...
    def draw(self, canvas):
        if not self.game_engine.lost:
            ...
            died_screen = pygame.image.load("texture/died.jpg")
            died_screen = pygame.transform.scale(died_screen, (640, 480))
            self.blit(died_screen, (0, 0))
